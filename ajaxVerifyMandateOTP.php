...
if(isset($_POST['otp']) && !empty($_POST['otp']))
{
    ...
    $loan_status=(isset($_SESSION['mandate_status']['loan_status']))?$_SESSION['mandate_status']['loan_status']:'';
    $ecs_status=(isset($_SESSION['mandate_status']['ecs_status']))?$_SESSION['mandate_status']['ecs_status']:'';
	...
	if (mysqli_num_rows($FetchDetailsQuery) > 0) {
        ...
        if(isset($_SESSION['mandate_status']['loan_status']) && $_SESSION['mandate_status']['loan_status']!='Active')
        {    
            echo "lsc";
            unset($_SESSION);
            session_destroy();
        }
        elseif(isset($_SESSION['mandate_status']['ecs_status']) && $_SESSION['mandate_status']['ecs_status']!='Active')
        {    
            echo "esc";
            unset($_SESSION);
            session_destroy();
        ...
        {
            $mandateDetailsResponse=getMandateDetails($_SESSION['mandate_status']['application_id'],$_SESSION['token']);
            ...
            if(!empty($mandateDetailsResponse))
            {
                ...
                if(isset($mandateDetails['achmandate_details']['error_code']) && $mandateDetails['achmandate_details']['error_code']=='01')
                {    
